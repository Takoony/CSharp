...
namespace get_data_czce
{
    ...
    public class ListOpenInterest
    {
        ...
        /// 
        /// </summary>
        public void save_list_open_rank(string instrument,string trading_day)
        {
            string file_path = instrument+"_Date\\" + trading_day + ".csv";
            ...
            for(int i=0;i<list_open_rank.Count;i++)
            {
                ...
                ctx += list_open_rank[i].change_hold_sell+ ","+ Utility.UtilityFun.format_date(trading_day)+"\r\n";
            }
            sw.Write(ctx);
            sw.Close();
            fs.Close();
        ...
        public void save_list_open_interest(string file_path, string trading_day)
        {
            ...
            for (int i = 0; i < list_open_interest.Count; i++)
            {
                ...
                ctx +=Utility.UtilityFun.format_date(trading_day);
                ctx += "\r\n";
                // FileStream fs = new FileStream(file_path,FileMode.Create);
